...
const TextForLastMatch = function (playerData, lastMatch) {
  ...
  if (!lastMatch.walkover) {
    if (lastMatch.won === 'true') {
      lst.lastMatch = `${playerData.properName} secured a victory in his last match on ${lastMatch.date}, defeating ${lastMatch.opponent} with a score of ${lastMatch.score}.`;

      lst.lastMatch_RO = `${playerData.properName} a obținut o victorie în ultimul său meci pe data de ${lastMatch.date}, învingându-l pe ${lastMatch.opponent} cu un scor de ${lastMatch.score}.`;
      ...
      lst.lastMatch_DE = `${playerData.properName} hat in seinem letzten Spiel am ${lastMatch.date} einen Sieg errungen, indem er ${lastMatch.opponent} mit einem Ergebnis von ${lastMatch.score} besiegt hat.`;

      lst.lastMatch_CZ = `${playerData.properName} dosáhl ve svém posledním zápase dne ${lastMatch.date} vítězství, když porazil ${lastMatch.opponent} se skóre ${lastMatch.score}.`;

      lst.lastMatch_PL = `${playerData.properName} zapewnił sobie zwycięstwo w swoim ostatnim meczu w dniu ${lastMatch.date}, pokonując ${lastMatch.opponent} z wynikiem ${lastMatch.score}.`;
    ...
    if (lastMatch.won === 'false') {
      lst.lastMatch = `${playerData.properName} suffered a defeat in his last match on ${lastMatch.date}, against ${lastMatch.opponent} with a score of ${lastMatch.score}.`;

      lst.lastMatch_RO = `${playerData.properName} a suferit o înfrângere în ultimul său meci pe data de ${lastMatch.date}, împotriva lui ${lastMatch.opponent} cu un scor de ${lastMatch.score}.`;
      ...
      lst.lastMatch_DE = `${playerData.properName} erlitt in seinem letzten Spiel am ${lastMatch.date} eine Niederlage gegen ${lastMatch.opponent} mit einem Ergebnis von ${lastMatch.score}.`;
